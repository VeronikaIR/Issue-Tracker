...
class ProjectRepository {
    ...
    //         RETURNING *
    //     `;
    //     const values = [project.project_key, project.name, project.description, project.creation_date, project.lead_user_id];
  ...
  `;
    ...
    //         RETURNING *
    //     `;
    //     const values = [project.name, project.description, project.creation_date, project.lead_user_id , id];
    //     const { rows } = await db.pool.query(query, values);
    //
    //     return new ProjectDto(rows[0].id, rows[0].project_key, rows[0].name, rows[0].description, rows[0].creation_date, rows[0].lead_user_id);
    // }
